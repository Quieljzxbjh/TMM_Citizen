...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        case 'login':
            ...
            // If this is a skip_otp_check request, only proceed if within grace period
            if (isset($sanitizedInput['skip_otp_check']) && !$hasRecentOTP) {
                sendResponse(false, 'OTP verification required', null, 401);
            }
            
            // For normal login, check OTP verification if not in grace period
            if (!isset($sanitizedInput['skip_otp_check']) && !$hasRecentOTP) {
            ...
            switch ($user['role']) {
                case 'commuter':
                    $redirectUrl = '../../commuter/index.php';
                    break;
                case 'operator':
                    $redirectUrl = '../../operator/dashboard.php';
                    break;
                case 'administrator':
                case 'admin':
                    // TODO: Add administrator dashboard path here
                    $redirectUrl = ''; 
                    break;
                default:
                    $redirectUrl = '../../commuter/index.php';
