...
function sendOTP(email, password) {
    // First try to login directly (might skip OTP if within grace period)
    fetch('login.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'login', email: email, password: password, skip_otp_check: true })
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            // Login successful without OTP (within grace period)
            showNotification('Login successful! Redirecting...', 'success');
            const redirectUrl = data.data?.redirect || '../../commuter/index.php';
            setTimeout(() => {
                window.location.href = redirectUrl;
            }, 1000);
        } else if (data.message === 'OTP verification required') {
            ...
            .then(credData => {
                if (credData.success) {
                    ...
                    showNotification('Sending OTP to your email...', 'info');
